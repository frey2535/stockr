...
export async function fetchBuildrProjects(companyId: string): Promise<{
  ...
  let lastError = key
    ? ""
    : "Buildr needs the shared service token. Set BUILDR_API_KEY on Stockr to the same STOCKR_BUILDR_SHARED_SECRET used on Buildr.";
      ...
      try {
        ...
        if (response.status === 401 || response.status === 403) {
          lastError = preferBuildrError(
            lastError,
            key
              ? "Buildr rejected the service token. Use the same STOCKR_BUILDR_SHARED_SECRET on Buildr and BUILDR_API_KEY on Stockr."
